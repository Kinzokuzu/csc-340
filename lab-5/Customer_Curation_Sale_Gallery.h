...
/*
 ...
 artworkListCurated; //the list of artworks that have ever been curated by the gallery. We are going
                     //to assume every artwork is unique. In other words, we don't have multiple
                     //copies of the same artwork.
 artworkListForSale; //the list of artworks that are available for purchase. note the difference
                     //between this list and the above.
...
public:
    ...
    // Note: make sure that you deduplicate the IDs as one artist may have multiple artworks for
    // sale
    vector<int> getIDsOfArtistsForSale() const;
    
    //Summarize a gallery's curated artworks (i.e., artworkListCurated) by  art type, style, and
    //subject.
    //ReportType is an enum class declared in the util_date_uniqueIDs.h. It consists of
    //{artType, artStyle, artSubject}.
    //For example, if reportType is ReportType::artType, this function will return a vector of the
    //following pairs: ("ArtType::painting", #), ("ArtType::photography", #),
    //("ArtType::drawing", #), ("ArtType::sculpture", #), ("ArtType::other", #).
    //You can call the string toStr_ArtType( ArtType ) function to convert an ArtType value to a
    //string.
    //You algorithm is going to replace the # with the actual number of artworks of the
    //corresponding artType.
    ...
    //add an artist to the artistList. Make sure that (1) this artist is not already on artistList
    //by checking the name & email combination; and (2) this artist has a valid ID by generating a
    //unique ID. Do nothing if the artist is already on the list.
    //returns artistID
    int addArtist(Artist artist);
    
    //adds a customer to the customerList. Make sure that (1) this customer is not already on
    //customerList by checking their name & email combination; and (2) this customer has a valid ID
    //by generating a unique ID. Do nothing if the artist is already on the list.Do nothing if this
    //customer is already on the list.
    //returns customerID.
    int addCustomer(Customer customer);
    

    //artwork curation: curate a new piece of artwork for the gallery. newItem.ID, newItem.artistID,
    //and artist.ID are all set to -1 (not available) initially. As a result, you’ll need to assign
    //this newItem a unique ID. Check if the artist is already on the artistList using their name
    //and email. If new, assign this artist a unique ID and add them to the artistList. If not new,
    //retrieve their ID from the artistList. You will also need to update the artworkListCurated and
    //the artworkListForSale.
    ...
    //artwork sale: sell a for-sale artwork to a customer. customer.ID is set to -1 initially. So
    //you will need to find out if this customer is new using their name and email address. If new,
    //assign this customer a unique ID and add them to the customerList. If not, retrieve their ID
    //from customerList.
    //You will also need to check if the specified artworkID is still for sale. If yes, update the
    //artworkListForSale by removing this artwork that was just sold, and update the salesRecords to
...
private:
    ...
    vector<Artist> artistsList; //the list of artists who have at least one artwork curated by the
                                //gallery
    vector<Customer> customersList; //the list of customers who have ever purchased an artwork from
                                    //the gallery
    vector<Artwork> artworksCurated; //the list of artworks that have ever been curated by the
                                     //gallery. We are going to assume every artwork is unique. In
                                     //other words, we don't have multiple copies of
                                     //the same artwork.
    vector<Artwork> artworksForSale; //the list of artworks that are available for purchase. note
                                     //the difference between this list and the above.
