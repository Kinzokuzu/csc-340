...
public:
    ...
    Artwork(); //default constructor: ID(-1),aritistID(-1), type(ArtType::painting),
               //style(ArtStyle::fineArt), subject(ArtSbuject::nature), yearMade(2022), width(0.00),
               //height(0.00),depth(0.00),price(0.00)
...
/*
 ...
 primaryArtType; //artist’s primary type of artworks, which can be one of the following {painting,
                 //photography, drawing, sculpture, other}
...
class Artist{
public:
    //default constructor: ID(-1),name("na"),email("na"),birthYear(1900),birthPlace("na"),
